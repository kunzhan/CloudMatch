...
from .eee import UltraLight_VM_UNet
...
class SingleNetwork(nn.Module):
    def __init__(self, num_classes, criterion, norm_layer, pretrained_model=None):
        ...
        self.backbone =UltraLight_VM_UNet(num_classes=2)
...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(pool)
        # pool = self.global_pooling_bn(pool) xtx qu diao le,bu zhi dao you mei you yingxiang
...
if __name__ == '__main__':
    model = Network(2, criterion=nn.CrossEntropyLoss(),
                    pretrained_model=None,
                    norm_layer=nn.BatchNorm2d).cuda()
    left = torch.randn(4, 3, 384, 384).cuda()
    right = torch.randn(4, 3, 384, 384).cuda()
